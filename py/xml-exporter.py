...
# These classes replace the original classes used in GenerateConfiguration.py.
# (because they could not be loaded by pypy.js)
def Element(name, attrs):
    return SubElement(None, name, attrs);
...
class SubElement:
    ...
    def tostring(self, tabs):
        ...
        for subelem in self.children:
            s += subelem.tostring("\t" + tabs)
